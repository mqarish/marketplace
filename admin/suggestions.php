...
if (isset($_POST['update_status'])) {
    ...
    if ($update_query->execute()) {
        $success_msg = 'تم تحديث حالة المقترح بنجاح';
    } else {
        $error_msg = 'حدث خطأ أثناء تحديث حالة المقترح';
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($delete_query->execute()) {
        $success_msg = 'تم حذف المقترح بنجاح';
    } else {
        $error_msg = 'حدث خطأ أثناء حذف المقترح';
    }
}

// تحديد عدد المقترحات لكل صفحة
$items_per_page = 10;
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="mb-0">إدارة المقترحات</h2>
                    <div>
                        <form class="d-flex" method="GET">
                            <select class="form-select me-2" name="status" onchange="this.form.submit()">
                                <option value="all" <?php echo $status_filter === 'all' || empty($status_filter) ? 'selected' : ''; ?>>جميع المقترحات</option>
                                ...
                                <option value="implemented" <?php echo $status_filter === 'implemented' ? 'selected' : ''; ?>>تم التنفيذ</option>
                ...
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <h5 class="card-title mb-0">قائمة المقترحات (<?php echo $total_items; ?>)</h5>
                    </div>
                    <div class="card-body">
                                        ...
                                        <?php foreach ($suggestions as $suggestion): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="?delete=<?php echo $suggestion['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('هل أنت متأكد من حذف هذا المقترح؟');">
                                                        <i class="bi bi-trash"></i>
                                                    </a>
                                                </td>
                                            </tr>
                                            
                                            <!-- Modal عرض وتحديث المقترح -->
                                                ...
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-primary text-white">
                                                            <h5 class="modal-title" id="viewSuggestionModalLabel<?php echo $suggestion['id']; ?>">تفاصيل المقترح #<?php echo $suggestion['id']; ?></h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="row mb-4">
                                                                <div class="col-md-6">
                                                                    <h6 class="fw-bold">معلومات المرسل:</h6>
                                                                    <p><strong>الاسم:</strong> <?php echo htmlspecialchars($suggestion['name']); ?></p>
                                                                    <p><strong>البريد الإلكتروني:</strong> <?php echo htmlspecialchars($suggestion['email']); ?></p>
                                                                    <p><strong>تاريخ الإرسال:</strong> <?php echo date('Y-m-d H:i', strtotime($suggestion['created_at'])); ?></p>
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <h6 class="fw-bold">حالة المقترح:</h6>
                                                                    <span class="badge <?php echo $status_class; ?> status-badge">
                                                                        <?php echo $status_text; ?>
                                                                    </span>
                                                                </div>
                                                            ...
                                                            <div class="mb-4">
                                                                <h6 class="fw-bold">نص المقترح:</h6>
                                                            ...
                                                            <form method="POST" action="">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="status<?php echo $suggestion['id']; ?>" class="form-label">تحديث الحالة:</label>
                                                                    <select class="form-select" id="status<?php echo $suggestion['id']; ?>" name="status">
                                                                        <option value="pending" <?php echo $suggestion['status'] === 'pending' ? 'selected' : ''; ?>>قيد المراجعة</option>
                                                                        <option value="reviewed" <?php echo $suggestion['status'] === 'reviewed' ? 'selected' : ''; ?>>تمت المراجعة</option>
                                                                        <option value="implemented" <?php echo $suggestion['status'] === 'implemented' ? 'selected' : ''; ?>>تم التنفيذ</option>
                                                                        <option value="rejected" <?php echo $suggestion['status'] === 'rejected' ? 'selected' : ''; ?>>مرفوض</option>
                                                                    </select>
                                                                </div>
                                                                
                                                                <div class="mb-3">
                                                                    <label for="admin_notes<?php echo $suggestion['id']; ?>" class="form-label">ملاحظات المسؤول:</label>
