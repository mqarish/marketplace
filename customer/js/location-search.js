...
// وظيفة إعادة التوجيه إلى صفحة set_location.php لتخزين الموقع
function redirectToSearchWithLocation(latitude, longitude) {
    ...
    // بناء عنوان URL لصفحة set_location.php
    const rootPath = document.querySelector('meta[name="root-path"]')?.getAttribute('content') || '';
    let locationUrl = `${rootPath}customer/set_location.php?`;
    
    // إضافة الموقع إلى URL
    locationUrl += `location=${latitude},${longitude}&`;
    
    // إضافة كلمة البحث إن وجدت
    if (searchValue) {
        locationUrl += `search=${encodeURIComponent(searchValue)}&`;
    }
    
    // إضافة نوع العرض
    locationUrl += `view=${viewType}`;
    
    // الانتقال إلى صفحة set_location.php
