<?php
// عرض رسالة فلاشية إذا كانت موجودة
if (isset($_SESSION['flash_message'])) {
    $flash_type = $_SESSION['flash_message']['type'];
    $flash_message = $_SESSION['flash_message']['message'];
    
    // عرض الرسالة الفلاشية
    echo "<div class='alert alert-{$flash_type} alert-dismissible fade show' role='alert' style='position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; width: 80%; max-width: 500px;'>";
    echo $flash_message;
    echo "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>";
    echo "</div>";
    
    // حذف الرسالة الفلاشية بعد عرضها
    unset($_SESSION['flash_message']);
}

// التحقق مما إذا كان المستخدم قد قدم مقترحًا
$suggestion_submitted = false;
$suggestion_error = '';

// معالجة نموذج المقترحات عند تقديمه
if (isset($_POST['submit_suggestion'])) {
    ...
    // التحقق من البيانات
    if (empty($name)) {
        $suggestion_error = 'يرجى إدخال الاسم';
    } elseif (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $suggestion_error = 'يرجى إدخال بريد إلكتروني صحيح';
    } elseif (empty($suggestion_text)) {
        $suggestion_error = 'يرجى إدخال نص المقترح';
    } else {
        // إدخال المقترح في قاعدة البيانات
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['flash_message'] = array(
                    'type' => 'success',
                    'message' => 'تم إرسال مقترحك بنجاح! شكرًا لك على مساهمتك في تحسين خدماتنا.'
                );
                ...
                $suggestion_error = 'حدث خطأ أثناء إرسال المقترح: ' . $stmt->error;
            }
        } else {
            // في حالة فشل التحضير
            $suggestion_error = 'حدث خطأ في النظام: ' . $conn->error;
        }
    }
}
?>

<!-- شاشة المقترحات المصغرة -->
<div class="suggestion-popup" id="suggestionPopup">
    ...
    <div class="suggestion-popup-body">
        <p>نرحب بمقترحاتك لتحسين خدماتنا</p>
...
<!-- نافذة نموذج المقترحات -->
<div class="modal fade" id="suggestionModal" tabindex="-1" aria-labelledby="suggestionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title" id="suggestionModalLabel">تقديم مقترح</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?php if ($suggestion_submitted): ?>
                    <div class="alert alert-success">
                        تم إرسال مقترحك بنجاح! شكرًا لك على مساهمتك في تحسين خدماتنا.
                    </div>
                    <script>
                ...
                <form id="suggestionForm" method="POST" action="">
                    <div class="mb-3">
                        <label for="suggestion_name" class="form-label">الاسم</label>
                    ...
                    <div class="mb-3">
                        <label for="suggestion_email" class="form-label">البريد الإلكتروني</label>
                    ...
                    <div class="mb-3">
                        <label for="suggestion_text" class="form-label">المقترح</label>
                        <textarea class="form-control" id="suggestion_text" name="suggestion_text" rows="4" required></textarea>
                        <div class="form-text">يرجى تقديم مقترحك بوضوح. سيتم مراجعته من قبل فريقنا.</div>
                    </div>
                    <button type="submit" name="submit_suggestion" class="btn btn-orange">إرسال المقترح</button>
                </form>
            </div>
        </div>
    </div>
...
// عرض نافذة المقترحات المصغرة بعد فترة من تحميل الصفحة
...
// عرض النافذة المصغرة بعد 10 ثواني من تحميل الصفحة في كل مرة
setTimeout(showSuggestionPopup, 10000); // 10 ثواني
</script>
