    ...
    if (empty($title)) {
        ...
        if (empty($error_msg)) {
            try {
                ...
                // إضافة سجل في جدول offer_store_products
                ...
                if (isset($_POST['items']) && is_array($_POST['items'])) {
                    ...
                    foreach ($_POST['items']['product_id'] as $index => $product_id) {
                        ...
                        // إضافة سجل في جدول offer_products
                        $insert_offer_product_sql = "INSERT INTO offer_products (offer_id, product_id) VALUES (?, ?)";
                        $insert_offer_product_stmt = $conn->prepare($insert_offer_product_sql);
                        $insert_offer_product_stmt->bind_param("ii", $offer_id, $product_id);
                        
                        if (!$insert_offer_product_stmt->execute()) {
                            throw new Exception('حدث خطأ أثناء ربط المنتج بالعرض: ' . $insert_offer_product_stmt->error);
                        }
                    }
                } else {
